...
const BACKUP_PATH = path.join(BACKUP_DIR, 'recommendations.json');
...
async function readLocal(): Promise<any[]> {
    ...
    if (primary.length === 0) {
        ...
        if (backup.length > 0) {
            console.log('[recommendations] Primary empty — restoring from backup');
...
async function writeLocal(data: any[]): Promise<void> {
    try {
        ...
        console.error('[recommendations] Write error:', error);
    }
}
